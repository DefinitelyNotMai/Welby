...
type QuarterlyAssessmentResultProps = {
  isOpen: boolean;
  onClose: () => void;
};

export const QuarterlyAssessmentResult = ({
  isOpen,
  onClose,
}: QuarterlyAssessmentResultProps) => {
    ...
    const fetchTiseData = async () => {
      ...
      if (tdata.length > 0) {
        ...
        const conditionString = conditions
          .map((condition) => tdata[0][condition])
          .join(",");
  ...
  return (
    ...
    >
      ...
      <ModalContent
        ...
        minWidth="35%"
      >
        <ModalHeader color="#000000">TISE Result</ModalHeader>
